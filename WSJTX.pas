...
type
 ...
 type
  ...
  { Private 宣言 }
    ...
    PastWsjtXStatusMessage: TWSJTXStatusMessage;
    PastWSJTXDecodeMessage: TWSJTXDecodeMessage;
    ...
    procedure SetHeartbeatTimeout(const Value: Cardinal);        // 秒単位
  ...
  { Public 宣言 }
    ...
    property OmitDuplicate: boolean read FOmitDuplicate write SetOmitDuplicate;
    property Active: boolean read FActive;

    property IpAddress: string read FIpAddress write SetIpAddress;
    Property Port: TIdPort read FPort write SetPort;    // TIdPortはWord(2桁 整数型)
...
begin
  ...
  try
    ...
    PastWsjtxStatusMessage  := TWSJTXStatusMessage.Create;
    PastWSJTXDecodeMessage  := TWSJTXDecodeMessage.Create;

    FModeMap := TDictionary<string, string>.Create;
    // WSJT-X の Mode 1文字 → 実モード名
    FModeMap.Add('~', 'FT8');
    FModeMap.Add('+', 'FT4');
    FModeMap.Add('@', 'JT65');
    FModeMap.Add('$', 'JT9');
    ...
    FHeartbeatTimeout := 60;  // 秒単位 60秒ごとにHeartbeatのタイムアウトをチェック
    FOmitDuplicate    := false;

    Timer1 := TTimer.Create(nil);
    Timer1.OnTimer := Timer1Timer;
    Timer1.Enabled := false;
    Timer1.Interval := 3000;    // 3秒に一回Heartbeatチェックする
...
procedure TWSJTX.Open();
begin
  Timer1.Enabled := true;
  UDPServer.Active := True;
end;

procedure TWSJTX.Open(Port: TIDPort);
begin
  Timer1.Enabled := true;
  SetPort(Port);
  UDPServer.Active := True;
...
procedure TWSJTX.Close;
begin
  Timer1.Enabled := false;
  UDPServer.Active := False;
end;

procedure TWSJTX.Timer1Timer(Sender: TObject);
var
  CurrentTime: TTime;
  ElapsedTime: TTime;
  ElapsedSec: Cardinal;
begin
  ...
  ElapsedSec := trunc(ElapsedTime * 24 * 60 * 60);  // 秒単位にする
  with FWSJTXStoppedHeartbeatMessage do
    begin
    ProgramId := FWSJTXHeartbeatMessage.ProgramID;
    StopTime  := Time();
    end;
  if  ElapsedSec >= FHeartbeatTimeout then
    begin
...
procedure TWSJTX.SetHeartbeatTimeout(const Value: Cardinal);
begin
  FHeartbeatTimeout := Value;
end;


procedure TWSJTX.SetHexMessageEnable(const Value: boolean);
begin
  FHexMessageEnable := Value;
end;


procedure TWSJTX.SetIpAddress(const Value: string);
begin
  FIpAddress := Value;
end;


procedure TWSJTX.SetOmitDuplicate(const Value: boolean);
begin
  FOmitDuplicate := Value;
end;


procedure TWSJTX.SetOnDecode(const Value: TNotifyEvent);
begin
  FOnDecode := Value;
end;


procedure TWSJTX.SetOnHeartbeat(const Value: TNotifyEvent);
begin
  FOnHeartbeat := Value;
end;


procedure TWSJTX.SetOnStatus(const Value: TNotifyEvent);
begin
  FOnStatus := Value;
end;


procedure TWSJTX.SetOnStoppedHeartbeat(const Value: TNotifyEvent);
begin
  FOnstoppedHeartbeat := Value;
end;


//procedure TWSJTX.SetstoppedHeartbeat(const Value: TNotifyEvent);
//begin
//  FOnstoppedHeartbeat := Value;
//end;

procedure TWSJTX.SetOnQSOLogged(const Value: TNotifyEvent);
begin
  FOnQSOLogged := Value;
end;


procedure TWSJTX.SetOnLoggedAdif(const Value: TNotifyEvent);
begin
  FOnLoggedAdif := Value;
end;


procedure TWSJTX.SetPort(const Value: TIdPort);
begin
  if Value < 1024 then
    begin
    MessageDlg('UDP Port番号エラー', mtError, [mbOK], 0, mbYes);
    exit;
    end;
  FPort := Value;
end;
...
procedure TWSJTX.UDPServerUDPRead(AThread: TIdUDPListenerThread;
  AData: array of Byte; ABinding: TIdSocketHandle);
var
  Bytes: TBytes;
  Msg: TObject;
...
function TWSJTX.MessageParser(const Bytes: TBytes): TWSJTXMessage;
var
  MsgType: Cardinal;
  Pos: integer;
begin
...
procedure TWSJTX.ParseHeartbeat(Bytes: TBytes);
var
  Pos: Integer;
begin
  Pos := 12;
...
procedure TWSJTX.ParseStatus(const Bytes: TBytes);
var
  Pos: Integer;
//  DeCall, DeGrid: string;
begin
...
//  Big EndingからLittle Endingへ変換する
    ...
    Mode := ReadString(Bytes, Pos);        // Mode(FT-8など)
    ...
    DeCall := ReadString(Bytes, Pos);      // 自分のCallsign
    DeGrid := ReadString(Bytes, Pos);      // 自分のグリッドスクエア
    DxGrid := ReadString(Bytes, Pos);      // 相手のグリッドスクエア
    end;
  FHexMessage := '';
  if HexMessageEnable then
    FHexMessage := MessToHex(Bytes);
...
procedure  TWSJTX.ParseDecode(Bytes: TBytes);
var
  Pos: Integer;
  qTime: cardinal;
begin
  ...
  with FWSJTXDecodeMessage do
    ...
    Time  := qTime / 86400000.0;                  // ms単位から時刻型に変換
    Time  := TTimeZone.Local.ToLocalTime(time);   // UTCからJSTに変換
    ...
    MessageText   := ReadString(Bytes, Pos);   // WSJT-Xがデコードしたメッセージ　
                                                    // 例 "E44K R3OZ 73","CQ BY6PWC OM64"など
...
procedure TWSJTX.ParseQSOLogged(Bytes: TBytes);
var
  Pos: Integer;
begin
  Pos := 12; // Magic + Schema + MsgType の後
  ...
  with FWSJTXQsoLoggedMessage do
    ...
    DateTimeOff   := TTimeZone.Local.ToLocalTime(DateTimeOff);   // UTCからJSTに変換
    ...
    DateTimeOn    := TTimeZone.Local.ToLocalTime(DateTimeOn);   // UTCからJSTに変換
    end;
  FHexMessage := '';
...
procedure TWSJTX.ParseLoggedADIF(Bytes: TBytes);
var
  Pos: Integer;
  d: string;
  t: string;
...
begin
  ...
  with FWSJTXLoggedAdifMessage do
    ...
    dt            := TTimeZone.Local.ToLocalTime(dt);           // UTCからJSTに変換
    qso_date      := DateOf(dt);
    time_on       := Timeof(dt);
    ...
    freq          := Trunc(StrToFloat(f) * 0.01) / 0.01;        // 18.102018を18.10に切り捨て
...
function TWSJTX.MessToHex(const Bytes: TBytes): string;
var
  hex: string;
  i: integer;
begin
...
function TWSJTX.ReadString(const Bytes: TBytes; var Pos: Integer): string;
var
  Len, i, n: Int32;
  SBytes: TBytes;
  s: string;
...
function TWSJTX.ReadInt64BE(const Bytes: TBytes; var Pos: Integer): Int64;
var
  n: int32;
  v: UInt64;
begin
...
function TWSJTX.ReadInt32BE(const Bytes: TBytes; var Pos: Integer): Int32;
var
  n: int32;
  v: Int32;
begin
...
function TWSJTX.ReadUInt32BE(const Bytes: TBytes; var Pos: Integer): Cardinal;
var
  n: int32;
  v: UInt32;
begin
...
function TWSJTX.ReadUInt8(const Bytes: TBytes; var Pos: Integer): byte;
var
  n: int8;
  v: UInt8;
begin
...
function TWSJTX.ReadFloatBE(const Bytes: TBytes; var Pos: Integer): Single;
var
  n: Int32;
  v: UInt32;
  f: Single absolute v;
begin
  ...
  // UInt32 のビットパターンを Single として解釈
  Result := f;
end;

function TWSJTX.ReadDoubleBE(const Bytes: TBytes; var Pos: Integer): Double;
var
  n: Int32;
  v: UInt64;
  d: Double absolute v;
...
function TWSJTX.ReadBoolean(const Bytes: TBytes; var Pos: Integer): boolean;
var
  n: Int32;
  b: boolean;
begin
...
function TWSJTX.ReadAdifElement(Adif: string; Pattern: string): string;
var
  p: string;
  m: TMatch;
begin
...
function TWSJTX.DecodeModeSymbol(const Symbol: string): string;
begin
  if FModeMap.ContainsKey(Symbol) then
    Result := FModeMap[Symbol]
  else
    Result := Symbol;  // 未知のモードはそのまま返す
end;


function TWSJTX.QDateTimeToTDateTime(const Bytes: TBytes; var Pos: Integer): TdateTime;
var
  d: Int64;
  t: Uint32;
  dt: tDateTime;
...
begin
  ...
  DT := (d - 2415020.5) + (t / 86400000.0);   // ms単位から日付時刻型に変換
  dt := DT + 1 + 3/24;                        // 疑問点 なぜか1日3時間ずれている？

  timespec := Bytes[Pos];
  inc (Pos, 1);
//  offset := Byte(Pos);          // QDateTimeの仕様と異なっている。Offset,TimeZoneなし
//  inc (Pos, 1);
//
//  if offset = 3 then
//    begin
...
//    end;
  ...
  JST := TTimeZone.Local.ToLocalTime(DT);    // UTCをJST変換
